...
<template>
  <AppDialog :open="open" :title="$t('admin.teamsNotice.title')" @close="emit('close')">
    <div class="space-y-4">
      <div class="flex items-start gap-2.5 rounded-lg bg-rose-50 px-3.5 py-3">
        <AlertTriangle class="mt-0.5 h-5 w-5 shrink-0 text-rose-500" />
      ...
      <i18n-t keypath="admin.teamsNotice.body" scope="global" tag="p" class="text-sm leading-relaxed text-slate-600">
        <template #bot><span class="font-medium text-slate-800">{{ $t('admin.teamsNotice.botName') }}</span></template>
      </i18n-t>
    ...
    <template #footer>
      <button
        type="button"
        class="rounded-md px-3 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-100"
        @click="emit('close')"
      >
