...
9. [파일 첨부](#9-파일-첨부)
10. [인증 · 보안](#10-인증--보안)
11. [관리자 페이지 · 개인 설정](#11-관리자-페이지--개인-설정)
...
- 알림센터 드롭다운: 안읽음 개수 배지, **모두 읽기**, 클릭 시 해당 QA/프로젝트로 이동. **QA 상세 페이지를 열면 그 QA 의 안읽은 알림은 자동으로 읽음 처리**됩니다
...
`/admin` — **관리자 전용** (일반 멤버는 네비에 노출되지 않고 접근 시 대시보드로 리다이렉트). 상단 통계 카드 4종(전체 프로젝트 / 전체 QA / 진행중 프로젝트 / 긴급 QA) + 4개 탭:

| 탭 | 기능 |
|---|---|
| 프로젝트 관리 | 목록 · 상태 인라인 변경 · QA 수 |
| QA 관리 | 목록 · 우선순위/상태 인라인 변경 · 담당자 |
| 팀원 관리 | 추가/수정/삭제(본인 차단), **권한 부여(관리자/멤버)**, **비밀번호 초기화**, **Teams 발송 테스트** |
...
모든 멤버가 쓰는 **내 설정** — 네비바 톱니바퀴 아이콘으로 진입. 본인 알림 설정(종류별 토글 · 방해금지)과 MS Teams 연동(마스터 토글 · 봇 설치 안내 · 나에게 테스트 발송)을 관리합니다. 전역 설정인 GitHub 탭은 관리자 페이지에만 노출됩니다.

## 12. 다국어 · 다크모드

- **한국어/영어** — 브라우저 언어 자동 감지 + 쿠키 기억, 네비바·로그인 화면에서 즉시 전환
- 데모는 **시드 데이터까지 언어별 제공** — 영어 방문자는 첫 화면부터 영어 데이터로 체험
- **다크모드** — 시스템/라이트/다크 3단 선택, OS 설정 자동 추종, 첫 페인트 깜빡임(FOUC) 방지, 전 화면 지원
