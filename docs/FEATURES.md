...
- **앱 사이드바** — 위에 메뉴(대시보드 · 알림 · 관리), 아래에 프로젝트 목록. 📌 고정한 프로젝트가 먼저 오고, 완료 프로젝트는 흐리게, 빨간 숫자는 `수정필요` 건수. 프로젝트를 누르면 그 자리에서 펼쳐져 **개요 · 테스트 케이스 · 테스트 플로우 · 테스트 런** 하위 메뉴가 나옵니다. 프로젝트가 많으면 검색 아이콘으로 이름을 걸러 찾습니다
