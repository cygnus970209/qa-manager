...
- **앱 사이드바** — 위에 메뉴(대시보드 · 알림 · 관리), 아래에 프로젝트 목록. 📌 고정한 프로젝트가 먼저 오고, 완료 프로젝트는 흐리게, 빨간 숫자는 그 프로젝트의 **새 알림 수**(프로젝트를 열거나 알림을 읽으면 지워짐). 프로젝트를 누르면 그 자리에서 펼쳐져 **개요 · 테스트 케이스 · 테스트 플로우 · 테스트 런** 하위 메뉴가 나옵니다. 프로젝트가 많으면 검색 아이콘으로 이름을 걸러 찾습니다
  - 프로젝트를 **우클릭**하면 상단 고정/고정 해제 메뉴가 뜹니다. 제목 옆 ↕ 아이콘으로 **순서 변경**(드래그) — 나에게만 적용되고 서버에 저장되어 어느 기기에서나 같습니다. 고정 프로젝트는 항상 위
...
사이드바 알림 아래 **검색** 메뉴, 또는 어디서나 `⌘K`(Ctrl+K) 팔레트.

- **대상** — QA 항목(제목·설명·카테고리) · 코멘트 본문 · 프로젝트(이름·설명) · 업데이트(버전·제목·설명) · 테스트 케이스(제목·사전 조건·스텝)
- **페이지** — 종류 탭(전체/QA/코멘트/프로젝트/업데이트/테스트 케이스, 건수) + 프로젝트 필터, 결과는 제목 일치 → 관련도 → 최신순. 일치한 글자는 강조. `#123` 또는 숫자만 치면 그 번호의 QA 가 맨 앞
- **팔레트** — 8건까지 종류별로 묶어 보여주고 ↑↓·Enter 로 바로 이동, "모든 결과 보기"로 페이지 이동
- **이동** — QA 는 상세로, 코멘트는 그 코멘트 위치로(잠시 강조), 업데이트는 프로젝트 개요의 해당 아코디언으로, 테스트 케이스는 편집 창이 열린 상태로
- **인덱스** — 별도 검색 테이블(`search_document`)을 쓰고 원본이 바뀔 때 자동 갱신, 매일 새벽 4시에 전체를 다시 만들어 정합성을 보장. 관리자는 설정 > 검색 인덱스에서 현황 확인·다시 만들기 (→ [ARCHITECTURE.md](ARCHITECTURE.md) 검색 절)
...
- **웹앱 새 버전 감지** — 서버를 새로 배포하면(SSE 재연결 시점, 그 외 10분 주기 확인) 화면 아래에 "새 버전이 배포되었습니다 · 새로고침" 배너가 뜹니다. 누르지 않아도 다음 화면 이동 때 새 버전으로 다시 불러옵니다. 데스크톱 앱에서는 `⌘R`/`F5` 로도 새로고침할 수 있습니다
...
| 묶음 | 항목 |
|---|---|
| 사용자 설정 | **내 계정**(프로필 이미지 · 이름 · 비밀번호 변경) / **알림**(MS Teams 수신 마스터 토글 · 봇 설치 안내 · 나에게 테스트 발송 → 6단계 진단 모달, 종류별 토글, 방해금지 시간대 — 한 화면에서 저장) |
| 앱 설정 | **모양**(시스템/라이트/다크 테마) / **언어**(한국어/English) / **데스크톱 앱**(버전 · 업데이트 확인 · 알림 권한 — 데스크톱 앱에서만, 브라우저에서는 내려받기 안내) |
| 관리자 (ADMIN 만 노출) | **팀원 관리**(추가/수정/삭제(본인 차단), **권한 부여(관리자/멤버)**, **비밀번호 초기화**, **Teams 발송 테스트**) / **GitHub 연동**(GitHub App 원클릭 생성 → 연동 상태 · 연결 저장소 목록 · 연동 해제) / **검색 인덱스**(원본·색인 건수, 상태 검사로 누락·고아·내용 변경 찾기, 불일치만 복구 또는 전체 다시 만들기) |
